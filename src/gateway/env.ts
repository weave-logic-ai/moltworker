...
export function buildEnvVars(env: MoltbotEnv): Record<string, string> {
  ...
  // AI_GATEWAY_MODEL intentionally NOT passed — model config is managed
  // via R2/openclaw.json to prevent startup script from creating broken providers
  ...
  // MentraOS bridge (used by mentra-bridge.js in start-openclaw.sh)
  if (env.MENTRA_API_KEY) envVars.MENTRA_API_KEY = env.MENTRA_API_KEY;
